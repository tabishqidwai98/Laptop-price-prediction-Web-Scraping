...
def get_links_from_page(page=1):
    ...
    target = Tag(cls='s-desktop-width-max s-desktop-content s-opposite-dir s-wide-grid-style sg-row')
    items = Tag(cls='puisg-row')
    link = Tag('a', cls='a-link-normal s-underline-text s-underline-link-text s-link-style a-text-normal', output='href')

    '''
    the Scraper method get_repeating data isn't returning anything, its displaying none
    the tags are not correct might be
    or the function has some issues in the library dputils. 
    runs after many tries for the previous tags, specific tag i searched
    '''

    all_links = scraper.get_repeating_page_data(target = target, items = items, link=link)
    print(len(all_links))
    return all_links
...
def get_laptop_details(link):
    ...
    manufacturer = Tag('span', cls='a-size-base po-break-word')
    ...
    result =  scraper.get_page_data(brand=manufacturer, 
                       model=model_name, 
